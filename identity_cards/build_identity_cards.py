...
TEX_OUT_SIX = ROOT / "identity_card_sheet.tex"
...
CLASS_ABILITIES = {
    ...
    "P.I.": "Maintains the team's notebook and records wins, discoveries, and collaborations; valid entries require an opposing team signature.",
...
def format_card_title(value: str) -> str:
    formatted = latex_format(value, italicize_scientific=True)
    match = re.fullmatch(r"(.+?)\s*(\([^()]+\))", formatted)
    if not match:
        return formatted

    primary, parenthetical = match.groups()
    return (
        rf"{primary}\\[-0.01in]"
        rf"{{\fontsize{{6.6}}{{6.9}}\selectfont {parenthetical}}}"
    )
...
def generate_crops(slide_w: int, slide_h: int, records: list[SlideRecord]) -> None:
    ...
    for record in records:
        ...
        if final_path.exists() and final_path.stat().st_mtime >= PDF.stat().st_mtime:
            continue
...
def generate_tex_sixup(records: list[SlideRecord]) -> None:
    header = r"""% Generated by build_identity_cards.py.
...
"""
    ...
    page_open = r"""% One landscape page = 2 columns x 3 rows = 6 cards.
...
"""
    ...
    for idx, record in enumerate(records, 1):
        ...
        chunks.append(
            ...
            f"{{{card_number}}}"
            f"{{{latex_format(record.card_class)}}}"
            f"{{{format_card_title(record.name)}}}"
            f"{{{image_path}}}"
            f"{{{latex_format(record.description, italicize_scientific=True)}}}"
            f"{{{latex_format(class_ability_text(record.card_class))}}}"
        )
        if idx != len(records) and idx % 6 == 0:
            chunks.append(page_close)
            chunks.append("\\newpage")
            chunks.append(page_open)
    ...
    chunks.append(page_close)
    chunks.append(footer)
    temp_tex = TEX_OUT_SIX.with_suffix(".tmp.tex")
    temp_tex.write_text("\n".join(chunks), encoding="utf-8")
