...
OUTPUT_PDF = REPO_ROOT / "output" / "pdf" / "active_field_study_poster.pdf"
OUTPUT_PNG = REPO_ROOT / "output" / "pdf" / "active_field_study_poster.png"
...
ALERT_RED = ImageColor.getrgb("#8A2D2D")
WHITE = (255, 255, 255)
...
def build_poster() -> tuple[Path, Path]:
    ...
    draw.text((header_x, panel_top + 118), "CONTAINMENT DIVISION", font=header_font, fill=AGENCY_BLUE)
    draw.text((header_x, panel_top + 214), "Station 744 Habitat Access Restrictions", font=subhead_font, fill=AGENCY_BLUE)
    ...
    paste_image_contained(image, LOGO_PATH, watermark_box, alpha_scale=0.07)

    title_font = load_font(104, bold=True)
    body_copy = (
        "Entry into this chamber is prohibited except by direct authorization "
        "from the primary resident. Disturbance of the local ecosystem may "
        "trigger defensive behavior, observational hostility, or permanent "
    ...
    body_cluster_height = 268 + 86 + body_box_height
    title_top = int((PAGE_HEIGHT / 2.0) - (body_cluster_height / 2.0))
    draw_centered_text(draw, center_x, title_top, "DO NOT OPEN -", title_font, ALERT_RED, tracking=4)
    draw_centered_text(draw, center_x, title_top + 120, "ACTIVE FIELD STUDY", title_font, ALERT_RED, tracking=4)

    divider_y = title_top + 268
    draw.line((panel_left + 180, divider_y, panel_right - 180, divider_y), fill=ALERT_RED, width=5)

    body_box_top = divider_y + 86
    ...
    draw_centered_text(
        ...
        center_x,
        footer_line_y + 34,
        "Unauthorized incursion will be logged as a containment breach.",
        footer_font,
        ALERT_RED,
    )
