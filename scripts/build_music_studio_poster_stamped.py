...
OUTPUT_PDF = REPO_ROOT / "output" / "pdf" / "pelagic_acoustics_lab_poster_stamped.pdf"
OUTPUT_PNG = REPO_ROOT / "output" / "pdf" / "pelagic_acoustics_lab_poster_stamped.png"
...
STAMP_RED = ImageColor.getrgb("#B54A39")
WHITE = (255, 255, 255)
...
def add_soundwave(draw: ImageDraw.ImageDraw, left: int, right: int, base_y: int) -> None:
    step = 42
    amplitude_pattern = [0, -18, 24, -34, 16, -28, 36, -12, 0]
    points: list[tuple[int, int]] = []
    x = left
    index = 0
    while x <= right:
        amplitude = amplitude_pattern[index % len(amplitude_pattern)]
        points.append((x, base_y + amplitude))
        x += step
        index += 1

    draw.line(points, fill=GOLD, width=4)
    draw.line([(x, y + 18) for x, y in points], fill=LAB_LINE, width=2)


def add_stamp(canvas: Image.Image, center_x: int, center_y: int) -> None:
    stamp_width = 1500
    stamp_height = 320
    stamp = Image.new("RGBA", (stamp_width, stamp_height), (255, 255, 255, 0))
    stamp_draw = ImageDraw.Draw(stamp)

    border_color = (*STAMP_RED, 190)
    fill_color = (255, 255, 255, 0)
    stamp_draw.rounded_rectangle(
        (18, 18, stamp_width - 18, stamp_height - 18),
        radius=22,
        outline=border_color,
        width=14,
        fill=fill_color,
    )
    stamp_draw.line((56, 76, stamp_width - 56, 76), fill=border_color, width=8)
    stamp_draw.line((56, stamp_height - 76, stamp_width - 56, stamp_height - 76), fill=border_color, width=8)

    stamp_font = load_font(86, bold=True)
    draw_centered_text(
        stamp_draw,
        stamp_width / 2.0,
        96,
        "AUTHORIZED SONIC TESTING",
        stamp_font,
        border_color,
        tracking=3,
    )

    rotated = stamp.rotate(-13, resample=Image.Resampling.BICUBIC, expand=True)
    paste_x = int(center_x - (rotated.width / 2.0))
    paste_y = int(center_y - (rotated.height / 2.0))
...
def build_poster() -> tuple[Path, Path]:
    ...
    draw_centered_text(draw, center_x, title_top - 50, "PELAGIC ACOUSTICS", title_font, DEEP_SEA, tracking=4)
    draw_centered_text(draw, center_x, title_top + 70, "LAB", title_font, DEEP_SEA, tracking=6)

    divider_y = title_top + 268
    add_soundwave(draw, panel_left + 215, panel_right - 215, divider_y + 4)
    ...
    add_stamp(image, int(center_x + 30), int(title_top + 1100))
