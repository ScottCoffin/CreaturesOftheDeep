...
OUTPUT_PDF = REPO_ROOT / "output" / "pdf" / "marine_waste_export_facility_poster.pdf"
OUTPUT_PNG = REPO_ROOT / "output" / "pdf" / "marine_waste_export_facility_poster.png"
...
def build_poster() -> tuple[Path, Path]:
    ...
    draw.text((header_x, panel_top + 118), "SANITATION DIVISION", font=header_font, fill=AGENCY_BLUE)
    draw.text((header_x, panel_top + 214), "Station 744 Hygiene and Waste Management Unit", font=subhead_font, fill=AGENCY_BLUE)
    ...
    paste_image_contained(image, LOGO_PATH, watermark_box, alpha_scale=0.09)

    title_font = load_font(110, bold=True)
    body_copy = (
        "All researchers, cetaceans, cephalopods, and marine-adjacent organisms "
        "must complete waste export in accordance with Station 744 containment protocol."
    )
    body_font = load_font(55, bold=False)
    ...
    body_cluster_height = 304 + 82 + body_box_height
    title_top = int((PAGE_HEIGHT / 2.0) - (body_cluster_height / 2.0))
    draw_centered_text(draw, center_x, title_top, "MARINE WASTE", title_font, DEEP_SEA, tracking=5)
    draw_centered_text(draw, center_x, title_top + 126, "EXPORT FACILITY", title_font, DEEP_SEA, tracking=5)

    divider_y = title_top + 304
    ...
    body_text_top = body_box_top + ((body_box_height - body_text_height) // 2) - 12
    draw_centered_lines(
        draw,
    ...
    footer_copy = "Cross-species splash incidents must be self-reported."
    draw_centered_text(draw, center_x, footer_line_y + 34, footer_copy, footer_font, AGENCY_BLUE)
