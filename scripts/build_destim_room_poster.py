...
OUTPUT_PDF = REPO_ROOT / "output" / "pdf" / "low_stimulus_recovery_habitat_poster.pdf"
OUTPUT_PNG = REPO_ROOT / "output" / "pdf" / "low_stimulus_recovery_habitat_poster.png"
LOGO_PATH = REPO_ROOT / "images" / "logo_small.png"
WAVE_IMAGE_PATH = REPO_ROOT / "images" / "calm_waves.jpg"
...
SOFT_SEA = ImageColor.getrgb("#CFE5EA")
WHITE = (255, 255, 255)
...
def build_poster() -> tuple[Path, Path]:
    ...
    draw.text((header_x, panel_top + 118), "RECOVERY DIVISION", font=header_font, fill=AGENCY_BLUE)
    draw.text((header_x, panel_top + 214), "Station 744 Sensory Regulation Unit", font=subhead_font, fill=AGENCY_BLUE)

    title_font = load_font(102, bold=True)
    body_copy = (
        "This chamber is reserved for nervous system recalibration, social "
        "decompression, horizontal observation, and temporary withdrawal "
    ...
    wave_image_width = 1320
    wave_image_height = 880
    title_block_height = 252
    gap_image_to_title = 70
    gap_title_to_body = 82
    main_stack_height = wave_image_height + gap_image_to_title + title_block_height + gap_title_to_body + body_box_height
    main_stack_top = int((PAGE_HEIGHT / 2.0) - (main_stack_height / 2.0))

    wave_left = int(center_x - (wave_image_width / 2.0))
    wave_top = main_stack_top
    wave_right = wave_left + wave_image_width
    wave_bottom = wave_top + wave_image_height
    wave_frame_pad = 14
    draw.rectangle(
        (wave_left - wave_frame_pad, wave_top - wave_frame_pad, wave_right + wave_frame_pad, wave_bottom + wave_frame_pad),
        fill=PANEL_BG,
        outline=LAB_LINE,
        width=3,
    )
    paste_image_contained(image, WAVE_IMAGE_PATH, (wave_left, wave_top, wave_right, wave_bottom))

    title_top = wave_bottom + gap_image_to_title
    draw_centered_text(draw, center_x, title_top, "LOW-STIMULUS", title_font, DEEP_SEA, tracking=4)
    draw_centered_text(draw, center_x, title_top + 114, "RECOVERY HABITAT", title_font, DEEP_SEA, tracking=4)

    divider_y = title_top + 252
    draw.line((panel_left + 190, divider_y, panel_right - 190, divider_y), fill=CALM_TEAL, width=4)

    body_box_top = divider_y + gap_title_to_body
